...
function NewClothRequest() {
  const [request,setDonation]=useState({desc:"",quantity:"",requesttype:"cloth",source:"",mob_no:""})
  const [userData,setUserData]=useState({Did:""})
  const [validation,setValidation]=useState({quantity:"",vmob_no:""})
  const [dis,setDis]=useState("disabled")
  const [checked,setChecked]=useState(false)
  const [success,setSuccess]=useState("d-none")
  const [error,setError]=useState("d-none")
  const clothimg=useRef()

   const PostData=async (e)=>{
      e.preventDefault(); 
      
      const {desc,quantity,requesttype,source,mob_no}=request
      const imgdata=clothimg.current.files[0].name
      const res=await fetch('/requestitem',{
          method:"POST",
          headers:{
              "Content-Type":"application/json"
          },
          body:JSON.stringify({desc,quantity,requesttype,source,mob_no,imgdata})
      });
  ...
  useEffect(() => {
      if(request.desc!==""&&request.quantity!==""&&request.source!==""&&request.mob_no!=="")
      {
          if(validation.quantity===""&&validation.vmob_no==""&&checked)
          {
              setDis("")
          }
          else
  ...
  function changeHandler(e){
      ...
      {
          if(value<5)
              setValidation({...validation,quantity:"d-block"})
          else
    ...
    <div className="row">
        <div className="col-xl-10 m-auto">
            <form method="POST" className={`form-group bg-light p-3 ${signupstyles.signupformgroup}`}>
                <h2 className={`mb-5 ${signupstyles.h2}`}>Request For Cloth</h2>
                <div className="row">
                    <div className="m-auto col-md-10">
                    <div className="mb-3">
                            <label htmlFor="quantity" className="form-label">Requirement for how much quantity?</label>
                            <input type="number" value={request.quantity} className="form-control" name="quantity" id="quantity"  required onChange={e=>changeHandler(e)}/>
                            <span className={`form-text invalid-feedback ${validation.quantity}`} id="quantity">Have atleast 5</span>
                        </div>
                        <div className="mb-3">
                            ...
                            placeholder="Enter cloth Description" required onChange={e=>changeHandler(e)}/>
                        </div>
                       

                        <div className0="mb-3">
                            <label htmlFor="source" className="form-label">Destination Address</label>
                            <textarea value={request.source} className="form-control" name="source" placeholder="Enter Destination Address" id="source" required onChange={e=>changeHandler(e)}/>
                        </div>
                        <div className="mb-3 my-2">
                            <label htmlFor="file" className="form-label">Enter Location Image: </label>
                            <input className='mx-2' type="file" ref={clothimg} name="foodimage" id="foodimage" required onChange={e => changeHandler(e)} />
...
export default NewClothRequest