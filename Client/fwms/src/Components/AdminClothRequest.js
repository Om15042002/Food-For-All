...
import adminclothrequeststyles from './AdminClothRequest.module.css'
import image from "../Images/anytime.jpg";

const images=require.context('../Images',true)
var requests
function AdminClothRequest() {
  const [flag,setFlag]=useState(false)
    ...
    const assignTask=async (e)=>{
      try{
        ...
        body:JSON.stringify({volunteer:Volunteer,area:Area,itemId:ItemId,type:"Cloth"})
      });
    ...
    useEffect(async ()=>{
      try{
          const res=await fetch('/getclothrequests',{
          method:"GET",
          headers:{
              "Content-Type":"application/json"
          } 
  ...
  if(!flag){return <><h3 className='m-3'>Loading...</h3></>}
      ...
      requestsList=requests.datas.map(request=>
      {
        ...
        return(
          <>
        <div className={`row shadow my-5  ${adminclothrequeststyles.border}`}>
                  <div className={`col-6 mt-3 ${adminclothrequeststyles.img1}`}>
                    <img src={source} width="500vw" height="300vh"/>
                  </div>
                  <div className={`col-6 p-3 ${adminclothrequeststyles.responsive}`}>
                  <table className="table table-borderless text-capitalize m-0">
                  <tbody className="text-nowrap special">
                  <tr className={adminclothrequeststyles.tr}>
                  ...
                  <tr className={adminclothrequeststyles.tr}>
                   ...
                   <tr className={adminclothrequeststyles.tr}>
                    ...
                    <button type="button" className={`btn btn-red m-2 mt-4 ${adminclothrequeststyles.btnred}`}  data-bs-toggle="modal" data-bs-target="#exampleModal" onClick={e=>setItemId(request.item._id)}>Assign</button>
                  </tbody>
                  </table>
                </div>
                  {/* <div className="row g-0 my-3 mx-1">
                    <div className="col-2">
                      <button type="button" className={`btn btn-red ${adminclothrequeststyles.btnred}`}  data-bs-toggle="modal" data-bs-target="#exampleModal" onClick={e=>setItemId(request.item._id)}>Assign</button>
                    </div>
                  </div> */}
            </div> 
        ...
        <div className="modal fade mt-5" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div className="modal-dialog">
              <div className="modal-content">
                ...
                <form method="POST" className={`form-group m-lg-5 ${adminclothrequeststyles.adminformgroup}`}>
                ...
                <div className="modal-footer">
                  <button type="button" className={`btn btn-red book ${adminclothrequeststyles.btnred}`} data-bs-dismiss="modal">Close</button>
                  <button type="button" className={`btn btn-red book ${adminclothrequeststyles.btnred}`} data-bs-dismiss="modal" onClick={assignTask}>Assign</button>
                </div>
                </form>
              </div>
            </div>
...
export default AdminClothRequest